...
r1 = np.arange(len(controllerSDN))
r2 = [x + barWidth for x in r1]
r3 = [x + barWidth for x in r2]

plt.ylim(0.0, 100.0)

plt.bar(r1, ryu_dados, color=color[0], width=barWidth, label=controllerSDN[0], yerr=ryu_margem_erro)
#plt.plot(r1, ryu_dados, color=color[0])

plt.bar(r2, floodlight_dados, color=color[1], width=barWidth, label=controllerSDN[1], yerr=floodlight_margem_erro)
#plt.plot(r2, floodlight_dados, color=color[1])

plt.bar(r3, pox_dados, color=color[2], width=barWidth, label=controllerSDN[2], yerr=pox_margem_erro)
