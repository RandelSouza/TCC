...
bar1 = plt.bar(r1, ryu_dados, color=color[0], width=barWidth, label=controllerSDN[0], yerr=ryu_margem_erro, capsize=55,  error_kw={"capthick": 2, 'ecolor': '#FFFF00'}, edgecolor='white', align='center', hatch='/')
#plt.plot(r1, ryu_dados, color=color[0])

bar2 = plt.bar(r2, floodlight_dados, color=color[1], width=barWidth, label=controllerSDN[1], yerr=floodlight_margem_erro, capsize=55, error_kw={"capthick": 2, 'ecolor': 'black'}, edgecolor='white', align='center', hatch='+')
#plt.plot(r2, floodlight_dados, color=color[1])

bar3 = plt.bar(r3, pox_dados, color=color[2], width=barWidth, label=controllerSDN[2], yerr=pox_margem_erro, capsize=55,  error_kw={"capthick": 2, 'ecolor': 'blue'}, edgecolor='white', align='center', hatch='X')
...
for rect in bar1 + bar2 + bar3:
    ...
    plt.text(rect.get_x() + rect.get_width()/2.0, height, '%.3f' % float(height), ha='center', va='bottom', fontsize=20)
